...
- **Rutas**: `/login` (pública), `/dashboard/{resumen,operativo,clases}`, `/alumnos`/`/alumnos/:id`, `/clases` y `/cuotas` (protegidas con `authGuard`), `/portal` (sin proteger, mockeado).
- **Bug de fechas corregido (off-by-one por timezone)**: toda fecha-solo que viene del backend como ISO UTC medianoche (`fechaVencimiento`, `fechaAlta`) se mostraba un día antes en cualquier timezone negativo (ej. Argentina, UTC-3), porque el pipe `date` de Angular formatea en la zona horaria local del navegador por defecto. Se fuerza `:'UTC'` como tercer parámetro del pipe en todos esos casos (`cuotas-list`, `alumnos-list`, `alumno-detalle`, `portal-alumno`). **Ojo**: esto no aplica a fechas construidas en el cliente con el constructor local de `Date` (ej. `fechaMes()` en `cuotas-list.ts`, que arma `new Date(anio, mes - 1)`) ni a `Date.now()`/`new Date()` reales (ej. el "hoy" del Dashboard) — forzar `:'UTC'` ahí introduciría el bug al revés.
